...
AWS_ACCESS_KEY=$(aws secretsmanager get-secret-value --profile personal --region $REGION --secret-id $SECRET_NAME --query 'SecretString' --output text | jq -r .$KEY)
AWS_SECRET_ACCESS_KEY=$(aws secretsmanager get-secret-value --profile personal --region $REGION --secret-id $SECRET_NAME --query 'SecretString' --output text | jq -r .$SECRET_KEY)
